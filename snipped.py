...
def create_artwork(values):
    length = len(values)
    artwork = images[0][values[0] - 1]
    
    # special case for the helmet
    if values[13] == 14:
        artwork = Image.alpha_composite(artwork, images[13][values[13] - 1])
        length -= 1

    artwork = Image.alpha_composite(artwork, images[1][values[1] - 1])

    # head decoration
    if values[1] != 9 and values[8] in [0, 9, 21, 22, 23, 24, 41, 42] and values[12] not in [25, 27] and values[13] in [0, 10, 14, 15]:
        artwork = Image.alpha_composite(artwork, head_decoration[values[1] - 1])
    
    for i in range(2, length):
        try:
            if values[i] > 0:
                artwork = Image.alpha_composite(artwork, images[i][values[i] - 1])
        except:
